...
action = function(host, port)
  ...
  for _, cookie in ipairs(response.cookies) do
    if cookie.name:find("BIGipServer") then
      ...
      elseif string.sub(cookie.value, 1, 2) == "rd" then
        host = string.sub(cookie.value:match "o00000000000000000000ffff[a-fA-F0-9]+", 26, -1)
        port = string.sub(cookie.value:match "o%d+$", 2, -1)
        if host and port then
          host = tonumber(string.sub(host,1,2),16) .. "." .. tonumber(string.sub(host,3,4),16) .. "." .. tonumber(string.sub(host,5,6),16) .. "." .. tonumber(string.sub(host,7,8),16)
